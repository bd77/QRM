DEMderiv<-function(data,attr,method){
  ...
  moore<-function(neighb){   #method proposed by Moore et al. (1993) as it is described in Florisky (1998)
    #Node numbering for the 3x3 window
    # x-axis from Z1 to Z3
    # y-axis from Z1 to Z7
    #Z1    Z2    Z3
    #Z4    Z5    Z6
    #Z7    Z8    Z9
    z1<-neighb[1]
    ...
    p=(z6-z4)/(2*cellValue)           # central first derivative to x
    q=(z2-z8)/(2*cellValue)           # central first derivative to y
    r=(z4+z6-2*z5)/(cellValue^2)      # second derivative to x
    s=(z3+z7-z1-z9)/(4*(cellValue^2)) # central 1st derivative to x of central 1ste derivative to y
    t=(z2+z8-2*z5)/(cellValue^2)      # second derivative to y
    if(paste(attr)=="slope"){result= atan(sqrt(p^2+q^2)) }                 
    else{
      if(paste(attr)=="aspect"){result= 180-atan2(q,p)+90*(p/abs(p)) }                         
      else{
  ...
  # in later R versions the focal function needs a weights matrix instead of a number
  mw = matrix(1, nrow = 3, ncol = 3)
  # in the code below the part ",w=3" was substituted by "w=mw"
  
  if(paste(method)=="evans"){crop(focal(data,w=mw,fun=evans,pad=T,padValue=0),
                                  y=c(data@extent@xmin+cellValue,data@extent@xmax-cellValue,data@extent@ymin+cellValue,data@extent@ymax-cellValue))}
  else{
    if(paste(method)=="zev.tho"){crop(focal(data,w=mw,fun=zev.tho,pad=T,padValue=0),
                                      y=c(data@extent@xmin+cellValue,data@extent@xmax-cellValue,data@extent@ymin+cellValue,data@extent@ymax-cellValue))}
    else{
      if(paste(method)=="shary"){crop(focal(data,w=mw,fun=shary,pad=T,padValue=0),
                                      y=c(data@extent@xmin+cellValue,data@extent@xmax-cellValue,data@extent@ymin+cellValue,data@extent@ymax-cellValue))}
      else{
        if(paste(method)=="moore"){crop(focal(data,w=mw,fun=moore,pad=T,padValue=0),
                                        y=c(data@extent@xmin+cellValue,data@extent@xmax-cellValue,data@extent@ymin+cellValue,data@extent@ymax-cellValue))}
      }}}
